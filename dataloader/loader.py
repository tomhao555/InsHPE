...
class nyu_loader(loader):
    def __init__(self, root_dir, phase, aug_para=[10, 0.1, 180], img_size=128,
        ...
        #     row = line.split()
        #
        #     # 将列表转换为 NumPy 数组,并将其添加到 data 列表中
        #     row_array = np.array(row, dtype=float).reshape(-1, 3)
        #     tmp_data.append(row_array)
        #
        # # 将 data 列表转换为 NumPy 数组
        # self.all_joints_xyz = np.stack(tmp_data, axis=0)
        ...
        self.teacher_center_xyz = self.all_joints_xyz.mean(1)
        ...
        # N = 62757 if phase == 'train' else 8252  # 假设有100张图片
        # self.all_depth_path = [
        #     self.data_path + '/depth_1_{:07d}.png'.format(index + 1)
        #     for index in range(N)
        # ]
        #
        # self.all_mesh_xyz = [
        #     np.loadtxt('{}/mesh/mesh_{}_1.txt'.format(self.root_dir, index + 1)).reshape([778, 3])
        #     for index in range(N)
        # ]

        # sample = [1, 2, 6, 9, 100, 55]
        # self.all_depth_path = [self.all_depth_path[i] for i in sample]
        # self.all_mesh_xyz = [self.all_mesh_xyz[i] for i in sample]
        # self.all_joints_xyz = self.all_joints_xyz[sample]
        # self.all_joints_uvd = self.all_joints_uvd[sample]
        # self.center_xyz = self.center_xyz[sample]
        # self.test_cubesize = self.test_cubesize[sample]

    def __getitem__(self, index):
        img_path = self.data_path + '/depth_1_{:07d}.png'.format(index + 1)
        if not os.path.exists(img_path):
            print(img_path)
        depth = self.loader(img_path)   # 640 480
        teacher_depth = depth
        ...
        # img_path = self.all_depth_path[index]
        # mesh_xyz = self.all_mesh_xyz[index]
        ...
        center_xyz = self.center_xyz[index]      # 手中心在相机空间下的坐标 mm
        center_uvd = self.joint3DToImg(center_xyz)      # 手中心在大图上的坐标 pixel
        ...
        # np.savez('nyu_data_1.npz', imgD=imgD, pcl_sample=pcl_sample)
        ...
        # # 可视化nyu关节点与残缺深度图
        # img_show = vis_tool.draw_2d_pose(data, joint_img, 'mano')
        # # 将图像转换为 PIL 格式
        # img_to_save = Image.fromarray(img_show.astype(np.uint8))
        #
        # # 定义保存路径
        # save_path = f'your_path/Datasets/nyu/nyu_{index}.png'
        #
        # # 保存图像
        ...
        # teacher_depth_imge
        teacher_center_xyz = self.teacher_center_xyz[index]  # 手中心在相机空间下的坐标 mm
        teacher_center_uvd = self.joint3DToImg(teacher_center_xyz)  # 手中心在大图上的坐标 pixel

        teacher_gt3Dcrop = joint_xyz - teacher_center_xyz.reshape(1, 3)
        teacher_depth_crop, trans = self.Crop_Image_deep_pp(teacher_depth, teacher_center_uvd, cube_size,
                                                            (self.img_size, self.img_size),
                                                            self.paras)
        if self.phase == 'train':

            teacher_imgD, _, _, teacher_cube, teacher_com2D, teacher_M, _ = self.augmentCrop(teacher_depth_crop, teacher_gt3Dcrop, teacher_center_uvd,
                                                                     self.cube_size,
                                                                     trans, mode, off, rot, sc, self.paras)

        else:
            teacher_imgD = self.normalize_img(teacher_depth_crop.max(), teacher_depth_crop, teacher_center_xyz, cube_size)
            teacher_cube = np.array(cube_size)
            teacher_com2D = teacher_center_uvd
            teacher_M = trans

        teacher_com3D = self.jointImgTo3D(teacher_com2D)

        # techer_pcl
        teacher_pcl = self.getpcl(teacher_imgD, teacher_com3D, teacher_cube, teacher_M)
        pcl_index = np.arange(teacher_pcl.shape[0])
        pcl_num = teacher_pcl.shape[0]
        if pcl_num == 0:
            teacher_pcl_sample = np.zeros([self.sample_num, 3])
        else:
            if pcl_num < self.sample_num:
                tmp = math.floor(self.sample_num / pcl_num)
                index_temp = pcl_index.repeat(tmp)
                pcl_index = np.append(index_temp, np.random.choice(pcl_index, size=divmod(self.sample_num, pcl_num)[1],
                                                                   replace=False))
            select = np.random.choice(pcl_index, self.sample_num, replace=False)
            teacher_pcl_sample = teacher_pcl[select, :]

        teacher_data = torch.from_numpy(teacher_imgD).float()
        teacher_data = teacher_data.unsqueeze(0)
        teacher_pcl_sample = torch.from_numpy(teacher_pcl_sample).float()
        teacher_center = torch.from_numpy(teacher_com3D).float()
        teacher_M = torch.from_numpy(teacher_M).float()
        teacher_cube = torch.from_numpy(teacher_cube).float()
        ...
        #
        # # 定义保存路径
        # save_path = f'your_path/Datasets/IncNYU/IncNYU_{index}.png'
        #
        # # 保存图像
        # img_to_save.save(save_path)
        # print(f"Image saved to {save_path}")
        #
        # img_show = vis_tool.draw_2d_pose(teacher_data, joint_img, 'mano')
        # # 将图像转换为 PIL 格式
        # img_to_save = Image.fromarray(img_show.astype(np.uint8))

        # 定义保存路径
        # save_path = f'your_path/Datasets/IncNYU/NYU_{index}.png'
        #
        # # 保存图像
        # img_to_save.save(save_path)
        # print(f"Image saved to {save_path}")
        ...
        if self.phase == 'train':
            return data, pcl_sample, joint, joint_img, center, M, cube, cam_para, cube_size[0], \
               teacher_data, teacher_pcl_sample, teacher_center, teacher_M, teacher_cube
        else:
            return data, pcl_sample, joint, joint_img, center, M, cube, cam_para, cube_size[0]
...
#IncNYU_loader
class IncNYU_pure_loader(loader):
    def __init__(self, root_dir, phase, aug_para=[10, 0.1, 180], img_size=128,
                 cube_size=[250, 250, 250], center_type='refine', joint_num=21, loader=nyu_reader):
        super(IncNYU_pure_loader, self).__init__(root_dir, phase, img_size, center_type, 'IncNYU')
        self.paras = (588.03, 587.07, 320., 240.)
        self.cube_size = np.array(cube_size)
        self.allJoints = True
        self.flip = 1
        ...
        #
        # 导入mask标签
        mask_path = '{}/inc_masks.txt'.format(data_path)
        ...
        # 将 data 列表转换为 NumPy 数组
        self.mask = np.stack(tmp_data, axis=0)
        ...
        if center_type =='refine':
            ...
            self.center_xyz = self.all_joints_xyz[:, 0, :]
        elif center_type =='joint_mean':
            self.center_xyz = self.all_joints_xyz.mean(1)
        elif center_type == 'random':
            self.center_xyz = self.all_joints_xyz.mean(1)
        ...
        self.test_cubesize[2440:, :] = self.test_cubesize[2440:, :] * 5.0 / 6.0
        # self.all_model_para = self.read_modelPara(root_dir, 0)
        # print("all_model_para_shape: {}" .format(self.all_model_para.shape))  #(72757, 62)
    ...
    def __getitem__(self, index):
        img_path = self.data_path + '/depth_{}.png'.format(index)
        if not os.path.exists(img_path):
            print(img_path)
        depth = self.loader(img_path)   # 640 480
        ...
        mask = self.mask[index].copy()
        # model_para = np.loadtxt('{}/para/para_{}_1.txt'.format(self.root_dir, index))
        # model_para = self.all_model_para[index]
        # print(model_para.shape)   #(62,)
        # mesh_xyz = np.loadtxt('{}/mesh/mesh_{}_1.txt'.format(self.root_dir, index)).reshape([778, 3])
        # print(mesh_xyz.shape)
        if self.phase == 'test':
            cube_size = self.test_cubesize[index]
        else:
            cube_size = self.cube_size
        ...
        #
        # # 定义保存路径
        # save_path = f'your_path/datasets/IncNYU/check/IncNYU_{index}.png'
        #
        # # 保存图像
        # img_to_save.save(save_path)
        # print(f"Image saved to {save_path}")
        ...
        #print(mask.shape)
        # return data, pcl_sample, joint, joint_img, center, M, cube, cam_para, mesh_xyz, mask
        # return data, pcl_sample, joint, joint_img, center, M, cube, cam_para, cube_size[0], model_para, mesh_xyz
        return data, pcl_sample, joint, joint_img, center, M, cube, cam_para, cube_size[0], mask

    def __len__(self):
        return len(self.all_joints_xyz)

#IncNYU_loader_teacher_encoder
class IncNYU_loader(loader):
    def __init__(self, root_dir, teacher_root_dir, phase, aug_para=[10, 0.1, 180], img_size=128,
                 cube_size=[250, 250, 250], center_type='joint_mean', joint_num=21, loader=IncNYU_reader, flag=False):
        super(IncNYU_loader, self).__init__(root_dir, phase, img_size, center_type, 'IncNYU')
        self.paras = (588.03, 587.07, 320., 240.)
        self.cube_size = np.array(cube_size)
        self.allJoints = True
        self.flip = 1
        ...
        #nyu data path teacher_root
        teacher_data_path = '{}/{}'.format(teacher_root_dir, self.phase)
        self.teacher_data_path = teacher_data_path
        nyu_labels_path = '{}/joint_data.mat'.format(teacher_data_path)
        self.nyu_labels = sio.loadmat(nyu_labels_path)

        self.nyu_all_joints_uvd = self.nyu_labels['joint_uvd'][0][:, joint_select, :][:, calculate, :]
        self.nyu_all_joints_xyz = self.nyu_labels['joint_xyz'][0][:, joint_select, :][:, calculate, :]
        self.nyu_all_joints_xyz = self.jointImgTo3D(self.joint3DToImg(self.nyu_all_joints_xyz, flip=-1))

        self.flag =  flag
        print(self.nyu_all_joints_xyz.shape)
        ...
        # # 将 data 列表转换为 NumPy 数组
        # self.all_joints_xyz = np.stack(tmp_data, axis=0)
        self.all_joints_xyz = self.nyu_all_joints_xyz
    ...
    def __getitem__(self, index):
        #load correspond nyu depth image
        global mode, off, rot, sc
        teacher_img_path = self.teacher_data_path + '/depth_1_{:07d}.png'.format(index + 1)
        if not os.path.exists(teacher_img_path):
            print(teacher_img_path)
        # print(teacher_img_path)
        teacher_depth = nyu_reader(teacher_img_path)
        ...
        depth = self.loader(img_path)   # 640 480
        joint_xyz = self.all_joints_xyz[index].copy()
        nyu_joint_xyz = self.nyu_all_joints_xyz[index].copy()
        # IncNYU_distance = np.linalg.norm(joint_xyz[20] - joint_xyz[15])  # 使用numpy计算欧氏距离
        # print(joint_xyz[20])
        # print(f"IncNYU关节间距: {IncNYU_distance:.2f} 单位")
        # nyu_distance = np.linalg.norm(nyu_joint_xyz[8] - nyu_joint_xyz[9])  # 使用numpy计算欧氏距离
        # print(nyu_joint_xyz[8])
        # print(f"nyu关节间距: {nyu_distance:.2f} 单位")
        ...
        if self.flag == False:
            data = teacher_data
            pcl_sample = teacher_pcl_sample
            center = teacher_center
            M = teacher_M
            cube = teacher_cube
